...
music_pause.addEventListener('click', function() {
    if (isMusicPlaying) {
        ...
        audioPlayer.play();
        isMusicPlaying = true;
        sendMessage({ type: 'play', currentTime: audioPlayer.currentTime });
    }
});
...
let createOffer = async () => {
    ...
    dataChannel = peerConnection.createDataChannel("syncChannel"); // 創建數據通道
...
function setupDataChannel() {
    dataChannel.onmessage = (event) => {
        const { type, currentTime } = JSON.parse(event.data);
        if (type === 'play') {
